...
class base:
   ...
   async def communicate_with_server(self, callback, check_name):
      global data
      global logger
      logger = methods.get_logger(check_name,self.LOG_FILE,self.FORMATTER,self.LOGGER_SCHEDULE,self.LOGGER_INTERVAL,self.LOGGER_BACKUPS) # Set up the logging  
   ...
   async def initialize_program(self, reader, writer):
      ...
      global config
      module_status_info = {}
      my_logger = methods.get_logger(self.LOGGER_NAME,self.LOG_FILE,self.FORMATTER,self.LOGGER_SCHEDULE,self.LOGGER_INTERVAL,self.LOGGER_BACKUPS) # Set up the logging
      my_logger.info("*********************************************************************************************************************************************")
      my_logger.info("5Eyes - Starting Display Status Checks")
      config = loadConfig(self.LOGGER_NAME) # Load the configuration information
      ...
      data = read_data(self.STATUS_FILE,self.LOGGER_NAME)
      ...
      ser = methods.setupSerialPort(config["baudrate"],self.LOGGER_NAME) # Initialise serial port
      device_found, valid_ports = self.search_devices(ser)
   ...
   def search_devices(self, ser, sleep_time, status): # Searches for all sender cards connected to each USB port (/dev/ttyUSBX) on the system
   ...
   def check_response(self, received_data):
      ...
      try:
         ...
         return False
   async def monitoring_log_output(self, message, exit_status, reader, writer):
      global data
      global logger
      #output to log monitor_log_file.log
      alarm = self.GOOD
      if self.WARNING in exit_status:
         alarm = self.WARNING
      elif self.CRITICAL in exit_status:
         alarm = self.CRITICAL
      else:
         alarm = self.UNKNOWN    
      try:
